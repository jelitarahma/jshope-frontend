...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      console.log('Edit page - Product data:', data); // Debug
      
      // API returns {product: {...}, variants: [...], images: [...]}
      const product = data.product || data;
      const apiVariants = data.variants || product.variants || [];

      // Convert product variants to form format
      const formVariants: FormVariant[] = apiVariants.length > 0
        ? apiVariants.map((v: any, idx: number) => {
            // Extract variant image from attributes
            const variantImageKey = Object.keys(v.attributes || {}).find(key => 
              key.startsWith('variant_') && key.endsWith('_images')
            );
            const variantImageUrl = variantImageKey ? v.attributes[variantImageKey] : '';
            
            // Filter out variant image from regular attributes
            const regularAttrs = Object.entries(v.attributes || {})
      ...
      // Ensure each variant has at least one attribute
      formVariants.forEach(v => {
        if (v.attributes.length === 0) {
          v.attributes.push({ key: '', value: '' });
        }
      });
      ...
      setFormData({
        ...
        thumbnail_url: product.thumbnail || '', // Use thumbnail directly, not getThumbnail
  ...
  // Variant management (same as create page)
  const addVariant = () => {
    setFormData({
      ...formData,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Handle main product image - match backend's new priority logic
      const isExternalUrl = formData.thumbnail_url && !formData.thumbnail_url.startsWith('/uploads/');
      
      console.log('Image submission debug:', {
        useUrlInput,
      ...
      // PRIORITY 1: External URL (new image from URL)
      if (isExternalUrl) {
        console.log('✅ Sending external thumbnail_url:', formData.thumbnail_url);
        fd.append('thumbnail_url', formData.thumbnail_url);
      } 
      // PRIORITY 2: File upload (new image file)
      else if (imageFile) {
        console.log('✅ Sending imageFile:', imageFile.name);
        fd.append('product_images', imageFile);
      }
      // PRIORITY 3: Keep old image (don't send anything)
      else {
        console.log('ℹ️ No image change - keeping old image');
      }
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 24px' }}>
      ...
      {/* Form Content - Reuse same structure as create page */}
      <div style={{
        backgroundColor: 'white',
        borderRadius: '16px',
        padding: '32px',
                  ...
                  <div>
                    ...
                    {formData.thumbnail_url && (
                      <div style={{ marginTop: '12px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: '12px', color: '#6b7280' }}>Current image</span>
                      </div>
                    )}
                  </div>
                )}
        ...
        {/* Step 2 & 3: Same as create page - variants and review */}
        {/* For brevity, I'll note that these are identical to create page */}
        {currentStep === 2 && (
          <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '12px', fontSize: '14px' }}>
                  ...
                  {formData.description && (
                    <>
                      ...
                      <span style={{ color: '#1f2937', whiteSpace: 'pre-wrap' }}>{formData.description}</span>
                    </>
                  )}
                </div>
